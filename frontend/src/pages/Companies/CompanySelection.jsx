...
export function CompanySelection() {
    ...
    return (
        <div className="space-y-6 pb-28 font-sans bg-slate-50 text-slate-800 relative">
            {/* Header info */}
            ...
            {errorMessage && (
                <div className="p-4 bg-red-50 border border-red-200 text-red-600 rounded-2xl text-xs font-semibold flex items-center gap-2.5 shadow-sm max-w-xl text-left">
                        ...
                        {filteredCompanies.map((company) => {
                            ...
                            return (
                                <div
                                    ...
                                    className={`
                                        flex items-center justify-between p-4 bg-white rounded-2xl border cursor-pointer select-none
                                        ${tracked ? "border-blue-200 shadow-sm" : "border-slate-200 hover:border-slate-300"}
                                ...
                                >
                                    <div className="space-y-0.5 max-w-[80%] text-left">
                                        ...
                                        <span className="block text-xs text-slate-500 font-medium truncate leading-relaxed">
                                            {company.company_name}
                                        </span>
                                    </div>
                                    ...
                                    <div className="shrink-0 pl-2">
                                        {itemLoading ? (
                                            ...
                                            <div className="w-6 h-6 rounded-lg bg-blue-600 text-white flex items-center justify-center border border-blue-600 transition">
                                                <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="3">
                                                    <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                                                </svg>
                                            </div>
                                        ) : (
                                            <div className="w-6 h-6 rounded-lg bg-white border border-slate-300 transition hover:border-slate-400" />
                                        )}
                                    </div>
                                </div>
                            );
            ...
            <div className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-white/95 backdrop-blur border border-slate-200 px-6 py-4 rounded-2xl shadow-xl z-30 flex items-center justify-between gap-8 max-w-md w-[calc(100%-2rem)]">
                <div className="text-left">
                    <span className="block text-[10px] text-slate-400 font-bold uppercase tracking-wider">Watchlist Selected</span>
                ...
                <button
                    ...
                    className="bg-blue-600 hover:bg-blue-700 text-white rounded-xl py-3 px-6 font-semibold text-sm transition duration-150 shadow-sm focus:outline-none"
                >
                    Save Watchlist
                </button>
            </div>
